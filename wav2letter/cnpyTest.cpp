...
int main(int argc, char** argv) {
  ...
  for (auto& sample : *ds) {
    auto input = sample[kInputIdx];    
    if (whether_threhold)
    {
      cnpy::NpyArray arr=cnpy::npy_load("/root/w2l/fft.npy");
      float* load_fft =arr.data<float>(); 
      LOG(INFO)<<"data's size is" << arr.shape.size() <<' '<<arr.shape[0]<<' '<<arr.shape[1]<<"\n";
      af::dim4 Dim=input.dims();
      int T=Dim[0];
      int K=Dim[1];
      LOG(INFO)<<"feature's size is"<<Dim;
      for (int i=0;i<T;i++)
      {  for (int j=0;j<K;j++)
	 {  input(i,j,af::span,af::span)=load_fft[i*T+j];
         }
      }
    }
    else{
    auto mean = af::mean<float>(input);
    auto stdev = af::stdev<float>(input);
    input = (input - mean) / stdev;
    }
    auto finalinput = input;

...
PowerSpectrum<T>::PowerSpectrum(const FeatureParams& params)
  ...
  auto nFFt = featParams_.nFft(); //512
...
//input: a strip of speech signal
template <typename T>
std::vector<std::vector<T>> PowerSpectrum<T>::myapply(const std::vector<T>& input) {
  auto frames = frameSignal(input, featParams_); //Return many frames signal
  if (frames.empty()) {
    return {};
  }
  return mypowSpectrumImpl(frames);
}


template <typename T>
std::vector<std::vector<T>> PowerSpectrum<T>::mypowSpectrumImpl(std::vector<T>& frames) {
  int64_t nSamples = featParams_.numFrameSizeSamples(); //400 dots
  int64_t nFrames = frames.size() / nSamples; // T frames of a strip speech signal
  int64_t nFft = featParams_.nFft(); //512<---400
  int64_t K = featParams_.filterFreqResponseLen(); //257
  ...
  std::vector<T> dft(K * nFrames);
  std::vector<T> outFft(2 * K * nFrames); //complex fft features
  ...
  for (size_t f = 0; f < nFrames; ++f) {
    auto begin = frames.data() + f * nSamples;
    {
      ...
      std::copy(outFftBuf_.begin(), outFftBuf_.begin() + 2 * K, outFft.begin() + f * 2 * K);
    }
  }
  result[0] = std::move(dft);  // T x K one dim vector
  result[1] = std::move(outFft); // T x 2K one dim vector
  return result;
}
...
int64_t PowerSpectrum<T>::outputSize(int64_t inputSz) {
  return featParams_.powSpecFeatSz() * featParams_.numFrames(inputSz); //257 x T
}

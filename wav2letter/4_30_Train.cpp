...
int main(int argc, char** argv) {
  ...
  auto train = [gradNorm,
    ...
    while (curEpoch < nepochs) {
      ...
      int numNoise = 200; //make 1000 noise sub-samples for the audio sample
      ...
      af::sync();
      std::ofstream preOutFile("/root/w2l/aboutM/preOutput.txt");
	...
	preOutFile.close();
      ...
      //detect which epoch to export fft
      const int sampleNum=10;
      int sampleindex[sampleNum];
      int top=0;
      //get the sequence like 0 ,500, 1000....
      for (int i=0;i< (numNoise/500);i++){
      sampleindex[top++]=i*500;
      if (top>=sampleNum) break;
      }
      top=0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
	//auto epsilon = (af::randn(noiseDims)) * 4268; 
        auto epsilon = fl::normal(noiseDims,4000,0).array(); //add noises
	LOG(INFO)<<"epsilon mean is:"<<af::mean<float>(epsilon);
	LOG(INFO)<<"epsilon stdev is:"<<af::stdev<float>(epsilon);
	//save last iter parameter:
	if (i == numNoise-1)
	{
	   std::ofstream epsfile("/root/w2l/aboutM/epsilon.txt");
	   if(epsfile.is_open())
  	   {
	...
	LOG(INFO)<<af::toString("rawinput 6 values:",rawinput(af::seq(6)));
        ...
        //output the value of the rawinput to the noiseinputdir
        /*
        if (sampleindex[top]==i){
	char noiseinputdir[100];
	sprintf(noiseinputdir,"/root/w2l/aboutM/noiseinput/Fft_%d.txt",sampleindex[top++]);
        std::ofstream noiseinput(noiseinputdir);
	if (noiseinput.is_open()){
	  noiseinput<<af::toString("noisefftinput is:",rawinput);
	  noiseinput.close();
	}
	...
	{
	  char outdir[100];
	  sprintf(outdir, "/root/w2l/aboutM/output_now/output_%d.txt", i);
	  std::ofstream outfile(outdir);
	  if(outfile.is_open())
	  {
	      outfile<<af::toString("net output is:", output.array());
	      outfile.close();
          }
	}
...
//////////////////////////////////////////////////////////////////////////////
	 af::sync();
        char dy_name[100];
        sprintf(dy_name,"/root/w2l/aboutM/dy__%d.txt", i);
        if (FLAGS_reportiters == 0) {

        std::ofstream mfile(dy_name);
        if(mfile.is_open())
        {
          mfile << af::toString("dy is:", dy);
          mfile.close();
        }
        }
	 ...
	 af::sync();
        char xgrad_name[100];
        sprintf(xgrad_name,"/root/w2l/aboutM/xgrad_%d.txt", i);
        if (FLAGS_reportiters == 0) {
        std::ofstream mfile(xgrad_name);
        if(mfile.is_open())
        {
          mfile << af::toString("xgrad is:", m);
          mfile.close();
        }
        }
...
/////////////////////////////////////////////////////////////////////////
	...
	//save first term grad and second term grad
	if(i%500 == 0)
	{
	  char gradfir[100];
	  char gradsec[100];
	  sprintf(gradfir, "/root/w2l/aboutM/output_now/firGrad_%d.txt", i);
	  sprintf(gradsec, "/root/w2l/aboutM/output_now/secGrad_%d.txt", i);
	  std::ofstream firfile(gradfir);
	  if(firfile.is_open())
	  {
	     firfile<<af::toString("fir grad is:", mGrad);
	     firfile.close();
          }
 	  std::ofstream secfile(gradsec);
          if(secfile.is_open())
	  {
	     secfile<<af::toString("sec grad is:", 2*lambda*m / (m_L2*m_L2));
    	     secfile.close();
          }
        }
        //print fir, sec, tot Grad norm of m
	firGradnormFile << af::norm(mGrad) << std::endl;
	secGradnormFile << af::norm(2*lambda*m/(m_L2*m_L2)) << std::endl;

        mGrad = mGrad - 2 * lambda * m / (m_L2 * m_L2);

	totGradnormFile << af::norm(mGrad) << std::endl;
...
/////////////////////////////////////////////////////////////////////         
	af::sync();
      	char lastm_name[100];
	sprintf(lastm_name,"/root/w2l/aboutM/lastm_%d.txt", i);
	if (FLAGS_reportiters == 0) {
